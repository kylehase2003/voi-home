...
export const PropertyCard = ({
  ...
  return (
    <Link to={`/property/${property.slug}`} className="h-full">
      <Card
        className={`group overflow-hidden hover:shadow-2xl transition-all duration-500 rounded-3xl h-full flex flex-col bg-card border-border/50 ${featured ? "md:hover:-translate-y-2 animate-fade-in" : "md:hover:-translate-y-1"}`}
        style={
          featured
            ? {
      ...
      >
        ...
        <div className={`relative ${featured ? "h-56 md:h-80" : "h-48 md:h-64"} overflow-hidden rounded-t-3xl`}>
          ...
          {property.property_type && (
            <div className="absolute top-2 left-2 md:top-4 md:left-4 max-w-[55%]">
              <Badge className="bg-gold text-white px-2 py-1 md:px-4 md:py-2 text-[10px] md:text-sm rounded-lg shadow-lg transition-colors duration-300 line-clamp-1">
          ...
          {property.construction_status && (
            <div className="absolute bottom-2 right-2 md:bottom-4 md:right-4">
              <Badge className="bg-[#941300] text-white font-bold px-2 py-1 md:px-4 md:py-2 text-[10px] md:text-sm rounded-lg shadow-lg">
        ...
        <CardContent className="p-0 flex flex-col flex-1">
          ...
          {property.installments_count && (
            <div className="border-b border-border/30">
              <div className="px-3 md:px-5 py-2 md:py-[12px]">
                <p
                  className="text-xs md:text-xl font-bold flex items-center justify-between transition-colors duration-500 text-foreground"
                >
          ...
          <div className="px-3 md:px-5 pt-3 md:pt-5 pb-2 md:pb-3 flex-1">
            <h3
              className={`text-base md:text-2xl font-bold mb-2 md:mb-3 group-hover:text-gold transition-colors duration-500 line-clamp-1 pb-1 text-foreground ${i18n.language === "ar" ? "font-arabic" : ""}`}
            >
            ...
            {property.description && (
              <p
                className="text-xs md:text-sm leading-relaxed line-clamp-2 md:line-clamp-3 mb-3 md:mb-4 transition-colors duration-500 text-muted-foreground"
              >
          ...
          <div
            className="px-3 md:px-5 pb-3 md:pb-5 pt-2 flex items-center justify-between gap-2 text-xs md:text-sm border-t transition-colors duration-500 text-muted-foreground border-border/30"
          >
